...
func _update_attack(delta: float) -> void:
	...
	if attack_clock >= 0.22 and attack_clock < 0.38:
		for enemy in get_tree().get_nodes_in_group("enemy"):
			...
			if offset.length() < 2.35 and facing.dot(offset.normalized()) > 0.18:
				enemy.take_damage(34.0, attack_serial)
	if attack_clock >= 0.62:
		is_attacking = false
...
func _animate(delta: float) -> void:
	...
	if is_dead:
		...
		var phase := clampf(attack_clock / 0.62, 0.0, 1.0)
		weapon_pivot.rotation.x = lerpf(-1.3, 1.65, smoothstep(0.15, 0.55, phase))
		body_pivot.rotation.y = sin(phase * PI) * 0.38
	else:
